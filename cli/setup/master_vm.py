...
def get_user_variables():
    ...
    if not ssh_key_path or not ssh_username:
        raise ValueError(
            colored(
                "SSH_PUBLIC_KEY_PATH and SSH_USERNAME must be defined in the .env file",
                "red",
            )
        )
    ...
    return {"ssh_keys": f"{ssh_username}:{ssh_public_key}"}
...
def terraform_run():
    ...
    if output_process.returncode != 0 or not output_process.stdout.strip():
        raise Exception(
            colored(
                "Failed to retrieve VM external IP. Ensure Terraform outputs are correctly defined.",
                "red",
            )
        )
